...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("on")
        .setDescription("Enable the goodbye system")
    ...
    .addSubcommand((sub) =>
      sub.setName("off").setDescription("Disable the goodbye system")
    )
    .addSubcommand((sub) =>
      sub
        .setName("text")
        .setDescription("Set custom goodbye text")
        .addStringOption((opt) =>
          opt
            .setName("message")
            .setDescription(
    ...
    .addSubcommand((sub) =>
      sub.setName("preview").setDescription("Preview the current goodbye card")
    )
    .addSubcommand((sub) =>
      sub
        .setName("status")
  ...
  async execute(interaction) {
    ...
    if (sub === "on") {
      ...
      const { error } = await supabase.from("leaver").upsert({
        guild_id: guildId,
        enabled: true,
    ...
    if (sub === "off") {
      ...
      if (error) {
        console.error("[Leaver/off]", error);
        return interaction.editReply({
          content: "❌ Failed to save configuration. Please try again.",
        });
      }
      ...
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription("Goodbye messages have been turned off.")
            .setTimestamp(),
        ],
      });
    }
    ...
    if (sub === "text") {
      ...
      if (error) {
        console.error("[Leaver/text]", error);
        return interaction.editReply({
          content: "❌ Failed to save configuration. Please try again.",
        });
      }
      ...
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor("#ed4245")
            .setTitle("✏️ Goodbye Text Updated")
            .addFields(
              { name: "New Text", value: message },
              {
                name: "Available Placeholders",
                value:
                  "`{user}` - Display name\n`{username}` - Username\n`{server}` - Server name\n`{count}` - Members remaining",
              }
            )
            .setTimestamp(),
        ],
    ...
    if (sub === "background") {
      ...
      if (error) {
        console.error("[Leaver/background]", error);
        return interaction.editReply({
          content: "❌ Failed to save configuration. Please try again.",
        });
      }
      ...
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor("#ed4245")
            .setTitle("🖼️ Goodbye Background Updated")
            .setImage(url)
            .setTimestamp(),
        ],
      });
    ...
    if (sub === "preview") {
      ...
      const attachment = new AttachmentBuilder(buffer, {
        name: "goodbye-preview.png",
      });

      return interaction.editReply({
        content: "**Preview of your goodbye card:**",
        files: [attachment],
      });
    }
    ...
    if (sub === "status") {
      ...
      if (!data) {
        return interaction.editReply({
          content: "❌ Goodbye system has not been configured yet.",
        });
      }
